...
-- Based on ncHoverBind by Nightcracker and Coote
-- Allows you to bind keys on buttons by hovering over them.

function ACTIONBAR:HoverBind()
	if not cfg.hoverBind then return end

	local bind, localmacros, frame = CreateFrame('Frame', 'FreeUIHoverBind', UIParent), 0

	SlashCmdList.MOUSEOVERBIND = function()
		if InCombatLockdown() then print(C.RedColor..ERR_NOT_IN_COMBAT) return end
		if not bind.loaded then
			...
			function bind:Deactivate(save)
				if save then
					AttemptToSaveBindings(FreeUIConfig['actionbar']['bindType'])
					print(C.RedColor..KEY_BOUND..'|r')
				else
					LoadBindings(FreeUIConfig['actionbar']['bindType'])
					print(C.RedColor..UNCHECK_ALL..'|r')
				end
				self.enabled = false
				self:HideFrame()
			...
			function bind:CallBindFrame()
				...
				local box = CreateFrame('CheckButton', nil, frame, 'InterfaceOptionsCheckButtonTemplate')
				box:SetSize(26, 26)
				box:SetPoint('RIGHT', text, 'LEFT', -5, -0)
				box:SetChecked(FreeUIConfig['actionbar']['bindType'] == 2)
				box:SetScript('OnClick', function(self)
					if self:GetChecked() == true then
						FreeUIConfig['actionbar']['bindType'] = 2
					else
						FreeUIConfig['actionbar']['bindType'] = 1
					end
				end)
				F.ReskinCheck(box)
			end
