...
function APPEARANCE:QuestTracker()
	...
	-- Fonts
	F.SetFont(ot.HeaderMenu.Title, C.font.header, 16, true)

	for _, headerName in pairs({'QuestHeader', 'AchievementHeader', 'ScenarioHeader'}) do
		local header = BlocksFrame[headerName]
		F.SetFont(header.Text, C.font.header, 16, true)
	end
	...
	hooksecurefunc(DEFAULT_OBJECTIVE_TRACKER_MODULE, 'SetBlockHeader', function(_, block)
		if not block.headerStyled then
			F.SetFont(block.HeaderText, C.font.normal, 14, true)
		...
		if block.lines then
			for _, line in pairs(block.lines) do
				if not line.styled then
					F.SetFont(line.Text, C.font.normal, 14, true)
